...
{
	$chkMobileQry = "SELECT count(pid_patient_id) from tbl_patient_details where pid_patient_mobile = '".$_POST['mobile']."'";
	$result = mysql_query($chkMobileQry);
	$row = mysql_fetch_row($result);
	if ($row[0]>0)
	{
		$_SESSION['error']['mobile'] = "Mobile - Already exists in database";	
	}
}

/*if(!isset($_POST['address1']) || trim($_POST['address1'])=='')
	$_SESSION['require']['address1'] = "Address 1 - Required Field Can't be blank";
...
if(trim($_POST['state'])!='')
if(!preg_match("/[[a-z]+[\s\_\-\.]*[a-z]*[\.]*[a-z]*]*$/i",$_POST['state']))
...
if(trim($_POST['city'])!='')
if(!preg_match("/[[a-z]+[\s\_\-\.]*[a-z]*[\.]*[a-z]*]*$/i",$_POST['city']))
...
// 	$relation = "INSERT INTO tbl_relationship_details (rs_relation_patientid, rs_relation_providerid, rs_relation_status) VALUES ('".$indipat_id."', '".$refuserid."', '1')";
	...
	{
		$json= '{ "serviceresponse" : { "servicename" : "Patient Details", "success" : "Yes","message" : "1" } }';
        $_SESSION['success'] = "Your patient was registered successfully";
	}
	else
	{
		$json= '{ "serviceresponse" : { "servicename" : "Patient Details", "success" : "No", "username" : "NULL",  "message" : "'.$error.'" } }';
	}
	//echo $json;
 //	exit;
