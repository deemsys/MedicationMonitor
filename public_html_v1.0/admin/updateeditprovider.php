...
//  echo $password; exit;
	$userdetail = "UPDATE tbl_user_details SET  ud_age='".$age."',ud_firstname = '".$firstname."', ud_lastname = '".$lastname."', ud_email_id = '".$email."', ud_skype_id = '".$skype."', ud_facetime_id = '".$facetime."', ud_mobile = '".$mobile."', ud_address1 = '".$address1."', ud_address2 = '".$address2."', ud_state = '".$state."', ud_city = '".$city."',ud_zipcode='".$zipcode."' WHERE ud_user_id =".$provider_id;




// echo $count; exit;
	if($query==mysql_query($userdetail))
	{
...
// 		$json 		= '{ "serviceresponse" : { "servicename" : "User Details", "success" : "Yes","message" : "1","userid " : "'.$row->ud_user_id.'","username" : "'.$row->ud_username.'","firstname" : "'.$row->ud_firstname.'","lastname" : "'.$row->ud_lastname.'","password" : "'.$row->ud_password.'","sex" : "'.$row->ud_sex.'","age" : "'.$row->ud_age.'","emailid" : "'.$row->ud_email_id.'","skypeid" : "'.$row->ud_skype_id.'","facetimeid" : "'.$row->ud_facetime_id.'","mobile" : "'.$row->ud_mobile.'","address" : "'.$row->ud_address.'","date" : "'.$row->ud_date.'","status" : "'.$row->ud_status.'" } }';
	...
	{
		$json = '{ "serviceresponse" : { "servicename" : "Update Provider Details", "success" : "No", "username" : "NULL",  "message" : "'.$error.'" } }';
	}
	//echo $json;
// 	exit;
 $_SESSION['success'] = "Provider Details was Updated successfully";
