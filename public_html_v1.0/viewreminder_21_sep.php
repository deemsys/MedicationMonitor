...
<?php
	...
	{
		$searchword.=' WHERE rd_reminder_name LIKE "%'.$_GET['remindersearch'].'%" OR rd_patient_name LIKE "%'.$_GET['remindersearch'].'%"';
	}
	else
	{
		$searchword = '';
	...
	$sqlreminder = "SELECT * FROM tbl_reminder_details".$searchword." LIMIT $start, $limit";
...
<?php
		for($i=0;$i<$ss;$i++)
{
	$sqlmedicin = "SELECT * FROM tbl_medication_details WHERE md_medicine_id = '".$medexp[$i]."';";
