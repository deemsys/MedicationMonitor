			...
			<table class="orders-table table">
				<tbody>
					...
					<tr>
						...
						<td><a href="facetime:<?php echo $patientrecords['pid_patient_facetimeid']; ?>" style="margin-left : 10px;"><img src="assets/img/Facetime.png" title="Face Time"></a></td>
					</tr>
...
<?php
	$sqlmed = "SELECT * FROM tbl_patientmedtaketime_details WHERE pmt_taketime_patientid =".$id;
	...
	for($j=0;$j<mysql_num_rows($querymed);$j++)
	{
		...
		$sqlmedicin = "SELECT * FROM tbl_medication_details WHERE md_medicine_id =".$medrecords['pmt_taketime_medicineid'];
...
<?php
	$sqlreminder = "SELECT * FROM tbl_reminder_details WHERE rd_patient_id = '".$id."';";

	$queryreminder = mysql_query($sqlreminder);

	for($i=1;$i<=mysql_num_rows($queryreminder);$i++)
	{
		$reminderrecords = mysql_fetch_array($queryreminder);
?>
				<tr>
					<td> <?php echo $i; ?> </td>
...
<?php
	$sqlmed = "SELECT * FROM tbl_patientmedication_details WHERE pm_patient_patientid =".$id;
	$querymed = mysql_query($sqlmed);
	$medrecords = mysql_fetch_array($querymed);

	$assid = $medrecords['pm_patient_assessmentid']; 
	$ass=str_replace('/',',',$assid);

	$assessexp = explode(",", $assid);
	$rr=count($assessexp)-1;

	for($k=0;$k<$rr;$k++)
	{
		$sql1 = "SELECT * FROM tbl_assessment_details WHERE ad_assessment_id = ".$assessexp[$k];
...
?>
				...
				<tr>
					<td> <?php echo $k+1; ?> </td>
					<td> <a href="viewpatassessment.php?id=<?php echo $_GET['id'];?>&type=<?php echo $assessexp[$k]; ?>" value="<?php echo $assessexp[$k]; ?>"><?php echo $assrecords['ad_assessment_name']; ?> </a></td>
					<td>  </td>
					<td> </td>
				</tr>
...
<?php
	$sqlapp = "SELECT * FROM tbl_appointment_details WHERE app_appointment_patientid =".$id;
...
<?php
	...
	$sql11 = "SELECT * FROM tbl_assessment_details";
...
<td>
	...
	<input type="checkbox" name="assessment[]" id="<?php echo $assess['ad_assessment_name']; ?>" value="<?php echo $assess['ad_assessment_id']; ?>">  <?php echo $assess['ad_assessment_name']; ?>
